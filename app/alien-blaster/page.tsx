...
export default function AlienBlasterPage() {
  ...
  useEffect(() => setBest(Number(localStorage.getItem('gamehub:alien-blaster-best') || 0)), []);
  ...
  const move = useCallback((dir: number) => { const s = stateRef.current; s.playerX = Math.max(28, Math.min(W - 28, s.playerX + dir * 32)); }, []);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (k === ' ' || k === 'f') shoot();
      if (k === 'enter' && stateRef.current.running === false) start();
    };
    const onUp = (e: KeyboardEvent) => { const k = e.key.toLowerCase(); if (k === 'arrowleft' || k === 'a') keys.current.delete('left'); if (k === 'arrowright' || k === 'd') keys.current.delete('right'); };
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (s.running) {
        ...
        if (s.enemies.some(e => e.alive && e.y > H - 115)) { s.lives = 0; s.running = false; setStatus('over'); }
  ...
  return <main className="alien-page"><div className="alien-shell"><div className="alien-top"><a href="/gamehub/">← GAMES</a><span>☄️ ALIEN BLASTER</span><b>WAVE {wave} · {score} · ❤️ {lives}</b></div><div className="alien-stage"><canvas ref={canvasRef} width={W} height={H} aria-label="Alien Blaster game"/></div><div className="alien-controls"><button onPointerDown={() => keys.current.add('left')} onPointerUp={() => keys.current.delete('left')} onPointerCancel={() => keys.current.delete('left')}>←</button><button className="fire" onPointerDown={shoot}>FIRE</button><button onPointerDown={() => keys.current.add('right')} onPointerUp={() => keys.current.delete('right')} onPointerCancel={() => keys.current.delete('right')}>→</button></div><div className="alien-actions"><button onClick={start}>{status === 'over' ? 'PLAY AGAIN' : status === 'playing' ? 'RESTART RUN' : 'START GAME'}</button><p>Move with A/D or ← → · Fire with Space · Destroy waves to increase the challenge.</p></div></div></main>;
}
