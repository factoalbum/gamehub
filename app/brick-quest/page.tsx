...
export default function BrickQuestPage(){
  ...
  const s=useRef({running:false,paused:false,score:0,best:0,lives:3,level:1,combo:0,maxCombo:0,paddle:W/2-PADDLE/2,wide:0,slow:0,won:false,bricks:makeLevel(1),balls:[{x:W/2,y:H-52,vx:2.8,vy:-4.2,stuck:true} as Ball],powers:[] as Power[]});
  const [ui,setUi]=useState({score:0,best:0,lives:3,level:1,combo:0,status:'ready' as 'ready'|'playing'|'paused'|'over'|'won'});
  const sync=useCallback(()=>{const x=s.current;setUi({score:x.score,best:x.best,lives:x.lives,level:x.level,combo:x.combo,status:x.won?'won':x.running?(x.paused?'paused':'playing'):'over'});},[]);
  const resetBall=useCallback(()=>{const x=s.current;x.balls=[{x:W/2,y:H-52,vx:(Math.random()>.5?1:-1)*(2.7+Math.random()*.5),vy:-4.2,stuck:true}];x.combo=0;},[]);
  const start=useCallback(()=>{const x=s.current;x.running=true;x.paused=false;x.won=false;x.score=0;x.lives=3;x.level=1;x.combo=0;x.maxCombo=0;x.paddle=W/2-PADDLE/2;x.wide=0;x.slow=0;x.bricks=makeLevel(1);x.powers=[];resetBall();sync();trackGame('game_start','brick-quest');},[resetBall,sync]);
  ...
  useEffect(()=>{const c=canvasRef.current;if(!c)return;const ctx=c.getContext('2d');if(!ctx)return;
    ...
    const tick=()=>{const x=s.current;if(x.running&&!x.paused){const speedScale=x.slow>0?.72:1;x.slow=Math.max(0,x.slow-1);x.wide=Math.max(0,x.wide-1);const pw=paddleWidth(x.wide),py=H-34;
        for(let bi=x.balls.length-1;bi>=0;bi--){const b=x.balls[bi];if(b.stuck){b.x=x.paddle+pw/2;continue;}b.x+=b.vx*speedScale;b.y+=b.vy*speedScale;if(b.x<7||b.x>W-7){b.x=Math.max(7,Math.min(W-7,b.x));b.vx*=-1;}if(b.y<7){b.y=7;b.vy=Math.abs(b.vy);}if(b.vy>0&&b.y+7>=py&&b.y-7<=py+13&&b.x>=x.paddle&&b.x<=x.paddle+pw){const hit=(b.x-(x.paddle+pw/2))/(pw/2),sp=Math.min(9,Math.hypot(b.vx,b.vy)*1.015);b.vx=sp*hit*.92;b.vy=-Math.sqrt(Math.max(7,sp*sp-b.vx*b.vx));b.y=py-8;}
          for(const br of x.bricks){if(br.hp<=0)continue;if(b.x+7>=br.x&&b.x-7<=br.x+br.w&&b.y+7>=br.y&&b.y-7<=br.y+br.h){br.hp--;x.combo++;x.maxCombo=Math.max(x.maxCombo,x.combo);const points=(br.hp===0?10:3)*x.level*Math.min(5,x.combo);x.score+=points;if(Math.random()<.11)x.powers.push({x:br.x+br.w/2,y:br.y+br.h,vy:2.2,kind:['wide','slow','multi'][Math.floor(Math.random()*3)] as Power['kind']});b.vy*=-1;break;}}
          if(x.bricks.every(br=>br.hp<=0)){if(x.level>=10){x.running=false;x.won=true;x.best=Math.max(x.best,x.score);localStorage.setItem('gamehub:brick-quest-best',String(x.best));trackGame('game_finish','brick-quest',{score:x.score,level:x.level,won:true,maxCombo:x.maxCombo});}else{x.level++;x.score+=100*x.level;x.bricks=makeLevel(x.level);resetBall();}}
          if(b.y>H+15){x.balls.splice(bi,1);if(!x.balls.length){x.lives--;x.combo=0;if(x.lives<=0){x.running=false;x.best=Math.max(x.best,x.score);localStorage.setItem('gamehub:brick-quest-best',String(x.best));trackGame('game_finish','brick-quest',{score:x.score,level:x.level,won:false,maxCombo:x.maxCombo});}else resetBall();}}
        }
