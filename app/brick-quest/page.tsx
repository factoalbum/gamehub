...
export default function BrickQuestPage(){
  ...
  const s=useRef({running:false,paused:false,score:0,best:0,lives:3,level:1,paddle:W/2-PADDLE/2,wide:0,slow:0,bricks:makeLevel(1),balls:[{x:W/2,y:H-52,vx:2.8,vy:-4.2,stuck:true} as Ball],powers:[] as Power[]});
  const [ui,setUi]=useState({score:0,best:0,lives:3,level:1,status:'ready' as 'ready'|'playing'|'paused'|'over'|'won'});
  const sync=useCallback(()=>{const x=s.current;setUi({score:x.score,best:x.best,lives:x.lives,level:x.level,status:x.running?(x.paused?'paused':'playing'):x.score>0&&x.level>10?'won':'over'});},[]);
  const resetBall=useCallback(()=>{const x=s.current;x.balls=[{x:W/2,y:H-52,vx:(Math.random()>.5?1:-1)*2.8,vy:-4.2,stuck:true}];},[]);
  const start=useCallback(()=>{const x=s.current;x.running=true;x.paused=false;x.score=0;x.lives=3;x.level=1;x.paddle=W/2-PADDLE/2;x.wide=0;x.slow=0;x.bricks=makeLevel(1);x.powers=[];resetBall();sync();},[resetBall,sync]);
  const move=useCallback((clientX:number)=>{const c=canvasRef.current;if(!c)return;const r=c.getBoundingClientRect(),px=(clientX-r.left)*W/r.width;const pw=xPaddle(s.current);s.current.paddle=Math.max(0,Math.min(W-pw,px-pw/2));const b=s.current.balls[0];if(b?.stuck){b.x=s.current.paddle+pw/2;}},[]);
  useEffect(()=>{const c=canvasRef.current;if(!c)return;const ctx=c.getContext('2d');if(!ctx)return;
    const draw=()=>{const x=s.current;ctx.clearRect(0,0,W,H);ctx.fillStyle='#080b11';ctx.fillRect(0,0,W,H);ctx.strokeStyle='rgba(120,228,255,.06)';for(let gx=0;gx<W;gx+=38){ctx.beginPath();ctx.moveTo(gx,0);ctx.lineTo(gx,H);ctx.stroke();}for(let gy=0;gy<H;gy+=38){ctx.beginPath();ctx.moveTo(0,gy);ctx.lineTo(W,gy);ctx.stroke();}
      ...
      const pw=xPaddle(x),py=H-34;ctx.fillStyle='#f7f9fc';ctx.beginPath();ctx.roundRect(x.paddle,py,pw,13,7);ctx.fill();for(const b of x.balls){ctx.fillStyle='#b7f34a';ctx.beginPath();ctx.arc(b.x,b.y,7,0,Math.PI*2);ctx.fill();}
      if(!x.running||x.paused){ctx.fillStyle='rgba(5,7,11,.68)';ctx.fillRect(0,0,W,H);ctx.fillStyle='#f7f9fc';ctx.font='800 31px sans-serif';ctx.textAlign='center';ctx.fillText(x.paused?'PAUSED':ui.status==='over'?'RUN OVER':ui.status==='won'?'QUEST CLEARED':'BRICK QUEST',W/2,H/2-10);ctx.fillStyle='#aeb7c8';ctx.font='600 14px sans-serif';ctx.fillText(x.paused?'P or tap pause to resume':ui.status==='over'?'Try again and chase a higher score.':'Clear 10 levels. Catch power-ups. Keep the streak alive.',W/2,H/2+20);}}
    const tick=()=>{const x=s.current;if(x.running&&!x.paused){const speedScale=x.slow>0?.72:1;x.slow=Math.max(0,x.slow-1);x.wide=Math.max(0,x.wide-1);const pw=xPaddle(x),py=H-34;
        for(let bi=x.balls.length-1;bi>=0;bi--){const b=x.balls[bi];if(b.stuck){b.x=x.paddle+pw/2;continue;}b.x+=b.vx*speedScale;b.y+=b.vy*speedScale;if(b.x<7||b.x>W-7){b.x=Math.max(7,Math.min(W-7,b.x));b.vx*=-1;}if(b.y<7){b.y=7;b.vy=Math.abs(b.vy);}if(b.vy>0&&b.y+7>=py&&b.y-7<=py+13&&b.x>=x.paddle&&b.x<=x.paddle+pw){const hit=(b.x-(x.paddle+pw/2))/(pw/2);const sp=Math.min(9,Math.hypot(b.vx,b.vy)*1.015);b.vx=sp*hit*.92;b.vy=-Math.sqrt(Math.max(7,sp*sp-b.vx*b.vx));b.y=py-8;}
          let hitBrick=false;for(const br of x.bricks){if(br.hp<=0)continue;if(b.x+7>=br.x&&b.x-7<=br.x+br.w&&b.y+7>=br.y&&b.y-7<=br.y+br.h){br.hp--;x.score+=br.hp===0?10*x.level:3*x.level;hitBrick=true;if(Math.random()<.11)x.powers.push({x:br.x+br.w/2,y:br.y+br.h,vy:2.2,kind:['wide','slow','multi'][Math.floor(Math.random()*3)] as Power['kind']});b.vy*=-1;break;}}if(hitBrick&&x.bricks.every(br=>br.hp<=0)){if(x.level>=10){x.running=false;x.best=Math.max(x.best,x.score);localStorage.setItem('gamehub:brick-quest-best',String(x.best));sync();}else{x.level++;x.score+=100*x.level;x.bricks=makeLevel(x.level);resetBall();}}
        ...
        sync();}draw();frame.current=requestAnimationFrame(tick);};frame.current=requestAnimationFrame(tick);return()=>{if(frame.current)cancelAnimationFrame(frame.current);};},[resetBall,sync,ui.status]);
  useEffect(()=>{const key=(e:KeyboardEvent)=>{if(['ArrowLeft','ArrowRight','a','d','A','D','p','P',' '].includes(e.key))e.preventDefault();if(e.key==='ArrowLeft'||e.key.toLowerCase()==='a')s.current.paddle=Math.max(0,s.current.paddle-34);if(e.key==='ArrowRight'||e.key.toLowerCase()==='d')s.current.paddle=Math.min(W-xPaddle(s.current),s.current.paddle+34);if(e.key===' '&&s.current.running&&s.current.balls[0]?.stuck){s.current.balls[0].stuck=false;}if(e.key.toLowerCase()==='p'&&s.current.running){s.current.paused=!s.current.paused;sync();}};window.addEventListener('keydown',key);return()=>window.removeEventListener('keydown',key);},[sync]);
  useEffect(()=>{const b=s.current;const v=Number(localStorage.getItem('gamehub:brick-quest-best')||0);b.best=v;setUi(u=>({...u,best:v}));},[]);
  const xPaddle=(x:{wide:number})=>PADDLE+(x.wide>0?48:0);
