...
export default function FootballRandom() {
  ...
  const reset = useCallback((start = true) => {
    players.current = makePlayers();
    ball.current = makeBall();
    phase.current = start ? 'playing' : 'ready';
    setUiPhase(phase.current);
    setScore([0, 0]);
    setWinner(0);
    setMessage('Tap your side to jump');
  ...
  useEffect(() => {
    ...
    const scorePoint = (scorer: 0 | 1) => {
      ...
      const s: [number, number] = [players.current[0].score, players.current[1].score];
      setScore(s);
      ...
      if (s[scorer] >= WIN) {
        ...
        ball.current = makeBall();
        ball.current.x = W / 2; ball.current.y = 170;
        setMessage(`GOAL! PLAYER ${scorer + 1}`);
      }
    };

    const drawPlayer = (p: Player, no: 1 | 2) => {
      const bob = Math.sin(p.phase) * 3;
      ctx.save(); ctx.translate(p.x, p.y + bob); ctx.fillStyle = no === 1 ? '#b7f34a' : '#78e4ff';
      ctx.beginPath(); ctx.arc(0, -12, 17, 0, Math.PI * 2); ctx.fill();
      ctx.fillRect(-16, 4, 32, 35);
      ctx.fillRect(-13, 39, 9, 23); ctx.fillRect(4, 39, 9, 23);
      ctx.restore();
    };
    const draw = () => {
      ctx.clearRect(0, 0, W, H);
      const g = ctx.createLinearGradient(0, 0, 0, H); g.addColorStop(0, '#162c20'); g.addColorStop(1, '#07100c'); ctx.fillStyle = g; ctx.fillRect(0, 0, W, H);
      ctx.fillStyle = '#14331f'; ctx.fillRect(0, FLOOR, W, H - FLOOR);
      ctx.strokeStyle = '#31533f'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(0, FLOOR); ctx.lineTo(W, FLOOR); ctx.stroke();
      ctx.strokeStyle = '#294936'; ctx.setLineDash([9, 11]); ctx.beginPath(); ctx.moveTo(W / 2, 0); ctx.lineTo(W / 2, FLOOR); ctx.stroke(); ctx.setLineDash([]);
      ctx.strokeStyle = '#dfe9e2'; ctx.lineWidth = 5; ctx.strokeRect(0, FLOOR - 100, 55, 100); ctx.strokeRect(W - 55, FLOOR - 100, 55, 100);
      drawPlayer(players.current[0], 1); drawPlayer(players.current[1], 2);
      const b = ball.current; ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(b.x, b.y, 14, 0, Math.PI * 2); ctx.fill();
      ctx.strokeStyle = '#b8c2c0'; ctx.lineWidth = 1; ctx.beginPath(); ctx.arc(b.x, b.y, 8, 0, Math.PI * 1.5); ctx.stroke();
    };
      ...
      if (phase.current === 'playing') {
        ...
        ps.forEach((p, i) => {
          ...
          if (Math.hypot(dx, dy) < 42) {
            ...
            b.vx = toward * (5.5 + Math.min(3, Math.abs(dx) / 10));
            b.vy = -7.5 - Math.max(0, -p.vy * .18);
            b.x = p.x + toward * 35; b.y = p.y + 4;
          }
        });
        if (b.y > FLOOR - 3 && b.x < 65) scorePoint(1);
        if (b.y > FLOOR - 3 && b.x > W - 65) scorePoint(0);
        if (b.y > FLOOR + 35) { b.x = W / 2; b.y = 170; b.vx *= -1; b.vy = -4; }
      }
  ...
  return <main className="random-football-page"><div className="random-football-shell">
    ...
    <section className="random-card"><div className="random-head"><span>ONE BUTTON · 2 PLAYER</span><strong>{message}</strong><small>Tap or hold your side. Both players can jump at the same time.</small></div>
      ...
      <div className="random-touch"><button className="p1-button" onPointerDown={(e) => { e.preventDefault(); jump(1); }}>P1 · JUMP</button><button className="p2-button" onPointerDown={(e) => { e.preventDefault(); jump(2); }}>P2 · JUMP</button></div>
