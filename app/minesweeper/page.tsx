...
export default function Minesweeper() {
  ...
  const reset = useCallback(() => { setBoard(empty()); setStarted(false); setStatus('ready'); setSeconds(0); setFlagMode(false); trackGame('game_restart', 'minesweeper'); }, []);

  const finish = useCallback((next: Cell[], won: boolean) => {
    setBoard(next); setStatus(won ? 'won' : 'lost'); setFlagMode(false);
  ...
  return <main className="mine-page">
    ...
    <section className="mine-panel">
      ...
      <div className="mine-actions"><button className="primary" onClick={reset}>{status === 'ready' ? 'START GAME' : 'NEW GAME'}</button><button className={`flag-toggle ${flagMode ? 'active' : ''}`} type="button" onClick={() => setFlagMode(mode => !mode)} aria-pressed={flagMode}>⚑ {flagMode ? 'FLAG MODE ON' : 'FLAG MODE'}</button><span>📱 Use Flag Mode to mark mines · 🖱️ Right-click also works</span></div>
