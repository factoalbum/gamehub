...
function ReflexGame({ game, onExit }: { game: Game; onExit: () => void }) {
  ...
  function hit() {
    ...
    if (state === 'go') { const value = Math.round(performance.now() - start.current); setTime(value); saveBest(value); setState('result'); }
  }
...
function SnakeGame({ game, onExit }: { game: Game; onExit: () => void }) {
  const [snake, setSnake] = useState<Point[]>(makeSnake); const [food, setFood] = useState<Point>({ x: 12, y: 7 }); const [dir, setDir] = useState<Point>({ x: 1, y: 0 }); const [score, setScore] = useState(0); const [running, setRunning] = useState(false); const [best, saveBest] = useBest('snake-best'); const dirRef = useRef(dir);
